...
export class DockerComposeClient extends ClientModule<DockerComposeClientConfig> implements HostModule<DockerComposeClientConfig>, NetworkModule<DockerComposeClientConfig>, LinkModule<DockerComposeClientConfig> {
	...
	async getContainers(): Promise<Container[]> {
		...
		if (!this.containers || now - this.containersCacheTimestamp > this.config.cacheTime) {
			this.containersCacheTimestamp = now
			this.containers = await this.httpClient.get<any[]>('/containers/json?all=true').then((containersResponse) => {
	...
	async getDockerNetworks(): Promise<DockerNetwork[]> {
		const now = new Date().getTime()
		if (!this.networks || now - this.networksCacheTimestamp > this.config.cacheTime) {
			this.networks = await this.httpClient.get<DockerNetwork[]>('/networks').then((networksResponse) => {
	...
	getNetworks(): Promise<Network[]> {
		return this.getDockerNetworks().then((networkResponse) => {
			...
			for (let network of networkResponse) {
				networks.push(Network.getInstance(network.Id, network.Name, network))
			}
	...
	getLinks(): Promise<Link[]> {
		return this.getContainers().then((containersResponse) => {
			let links: Link[] = []
			let dnsContainers: { [key: string]: {network?: Network, target: Host} } = {}
			for (let container of containersResponse) {
				for (let networkName in container.NetworkSettings.Networks) {
					...
					if (network.Aliases){
						for(let alias of network.Aliases) {
							let networkLink: Network | undefined
							if (network.NetworkID) {
								networkLink = Network.getInstance(network.NetworkID, networkName, network)
							}
							dnsContainers[alias] = {
								network: networkLink,
								target: this.getHostFromContainer(container),
							}
						}
					}
				}
			...
			for (let container of containersResponse) {
				if(this.config.linkIndicatorDnsEnvironmentVariable) {
					for (let envKeyValue of container.Config.Env) {
						...
						for(let dnsContainer in dnsContainers) {
							if(envValue[1].match(dnsContainer)){
								...
								const target = dnsContainers[dnsContainer].target
								const via = dnsContainers[dnsContainer].network
								...
								links.push({source, target, via: via? [via]: []})
							}
						}
