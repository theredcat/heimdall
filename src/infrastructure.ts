...
import { ClientModule } from './datasources/index.ts'
import { Host, HostActionStatus, HostModule } from './host.ts'
...
export class Infrastructure {
	...
	public addDataSource(module: ClientModule<any>) {
	...
	public update(autoArrangeNodes = false): Promise<boolean> {
		...
		for (const module of this.modules) {
			if ('getHosts' in module) {
				const hostsPromise = (module as HostModule<any>).getHosts();
				this.updateHosts(hostsPromise);
			}
		
			if ('getNetworks' in module) {
				const networksPromise = (module as NetworkModule<any>).getNetworks();
				this.updateNetworks(networksPromise);
			}
		
			if ('getLinks' in module && 'getHosts' in module) {
				const hostsPromise = (module as unknown as HostModule<any>).getHosts();
				const linksPromise = (module as LinkModule<any>).getLinks(hostsPromise);
				this.updateLinks(linksPromise);
			}
		}
