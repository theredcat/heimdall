...
export class Host {
	...
	dns: string[]
    state: HostState
	provider: HostModule<any>
	networks: { [key: string]: Network };

	constructor(id: string, name: string, dns: string[], state: HostState, provider: HostModule<any>, data: {[key: string]: any } ) {
		this.id = id
		this.name = name
		this.data = data
		this.dns = dns
...
export interface HostModule<C extends Record<string, Setting<string | number | boolean>>> extends ClientModule<C> {
